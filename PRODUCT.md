...
- Hosted runtime bridge handoff requires shared Postgres-backed command/payload stores and worker-owned claim/apply flow.
- Stale claimed provider-input commands fail closed with `hosted_runtime_bridge_non_idempotent_retry_blocked` (no blind provider-input retries).
