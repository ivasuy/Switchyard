...
R23 hosted note: `claude_code.sdk` hosted bridge support is shipped through existing hosted run input and approval resolution endpoints when runtime-bridge readiness checks pass (command store/outbox, ownership, quota/audit, worker claim, session reconciliation, approval sender, adapter capability). Server-to-worker handoff depends on shared Postgres-backed hosted runtime bridge command and payload stores. Stale claimed provider-input commands fail closed with `hosted_runtime_bridge_non_idempotent_retry_blocked` instead of blindly retrying provider input. Activation remains operator opt-in. Fake/no-spend remains default.
