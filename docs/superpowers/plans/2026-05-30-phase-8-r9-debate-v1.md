...
R9 treats a debate as a product workflow record, not a runtime adapter. The debate service first validates the entire create request, then creates a durable debate record, creates two normal fake deterministic runs through `RunService`, runs a sequential bounded round loop, routes every turn through `MessageRouter`, appends debate-scoped events, judges with a pure deterministic function, and writes a summary artifact through the existing artifact metadata and optional content stores. Body parsing, reserved-field rejection, fake-runtime validation, limit validation, and evidence id validation all occur before `DebateStore.create`. Unknown evidence and unsupported real runtimes must produce no debate, run, message, event, or artifact side effects.

```text
POST /debates?wait=1
  -> debate-routes validation and error envelope
  -> DebateService.createAndMaybeExecute
       -> parse body and reject reserved fields
       -> validate exactly two fake deterministic participants
       -> apply and validate R9 hard limits
...
- `packages/contracts/src/http-error.ts` - adds `debate_not_found` and ensures `evidence_not_found` remains in the closed REST error-code schema.
- `packages/contracts/test/contracts.test.ts` - proves R9 debate defaults/fields and both `debate_not_found` and `evidence_not_found` error codes parse without breaking existing contracts.
...
3. Extend `MessageRouter` without breaking public callers. Add optional `debateId` and `participantId` to `CreateMessageInput`. Add `createWithEvent(input): Promise<{ message: RoutedMessage; event: SwitchyardEvent }>` and make `create(input)` call it and return only `message`. When `debateId` is present, include `debateId`, `participantId`, and debate metadata in the emitted event. Compute event `sequence` from the next monotonic sequence in every scope the event belongs to: use max existing `sequence + 1` from `events.listByDebate(debateId)` and from `events.listByRun(eventRunId)` when the message also has a run id, then choose the higher value. This allows debate streams to have gaps while keeping both debate replay and participant run replay ordered by ascending sequence. For non-debate messages, preserve the existing run-scoped sequence behavior. Publish the event through the existing event bus path.
...
- `{ name: "contract_http_error_codes", lens: "happy", given: "httpErrorCodeSchema.parse for debate_not_found and evidence_not_found", expect: "both codes parse and existing error-code tests remain green" }`
- `{ name: "error_unsupported_participant_runtime", lens: "error_path", given: "participant runtime codex or claude_code", expect: "400 invalid_input and no debate/run/message/event/artifact side effects; assert all relevant stores remain at their pre-request counts" }`
...
- `{ name: "error_unknown_evidence", lens: "error_path", given: "evidenceIds contains evidence_missing", expect: "404 evidence_not_found and no debate/run/message/event/artifact side effects; assert all relevant stores remain at their pre-request counts" }`
...
- `{ name: "run_event_replay_order_with_debate_message", lens: "integration", given: "successful debate then GET /runs/:participantRunId/events for both participants", expect: "run replay remains ascending by sequence and coherent even when message.sent events carry debateId and participantId" }`
