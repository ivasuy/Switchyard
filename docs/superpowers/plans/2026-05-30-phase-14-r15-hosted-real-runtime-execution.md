...
- `packages/core/src/services/hosted-runtime-catalog.ts` - new shared closed catalog, config/gate validation helpers, real-mode predicates, runtime-mode manifests, hosted safety checks, and claim preparation.
...
7. When `prepareHostedRunForExecution` returns a mutated safe run, update the durable row before `startRun(run.id)` so the existing adapter sees defaulted `sandbox: "read-only"`.
8. Do not add a public sandbox API, public process API, public PTY API, generic process adapter, or generic PTY adapter.
  ...
  "exports": [
    {
      ...
      "kind": "constant",
      "signature": "Record<'fake.deterministic' | 'codex.exec_json' | 'claude_code.sdk' | 'opencode.acp', HostedRuntimeCatalogEntry>"
    },
    {
...
7. Guard `POST /runs/:id/input` for active hosted real runtime rows. If R15 has no hosted worker input bridge, return the existing adapter protocol error envelope with reason `hosted_input_unsupported` or `hosted_interaction_unsupported`; do not call `runService.sendInput` and do not return success while dropping input.
8. Guard `POST /runs/:id/cancel` for active hosted real runtime rows. If verified worker-owned cancellation is not implemented in this phase, return the existing adapter protocol error envelope with reason `hosted_cancel_unsupported`; do not report successful cancellation while provider execution may continue.
...
- [ ] Server registry lists `codex.exec_json`, `claude_code.sdk`, and `opencode.acp` with hosted placement `conditional` and self-hosted/staging limitations.
...
- [ ] Hosted real input/cancel routes fail visibly with the existing error envelope when no worker bridge is present.
...
| `/runs/:id/input` | hosted real active run without input bridge | `AdapterProtocolError` | return 409 `adapter_protocol_failed` with reason `hosted_input_unsupported` | client sees input not accepted |
...
| lists hosted real registry records | integration | GET `/runtime-modes` after app creation | records include three real modes with hosted `conditional` |
...
| input bridge unsupported | error_path | active hosted real run row then POST `/runs/:id/input` | 409 `adapter_protocol_failed`, `hosted_input_unsupported` |
| cancel bridge unsupported | error_path | active hosted real run row then POST `/runs/:id/cancel` | 409 `adapter_protocol_failed`, `hosted_cancel_unsupported` |
...
```json
{
  ...
  "imports_from_other_tasks": [
    ...
    },
    {
      "from_task": "P14-T1-hosted-runtime-policy",
      "name": "HOSTED_RUNTIME_CATALOG",
      "signature": "Record<string, HostedRuntimeCatalogEntry>"
    },
    {
      "from_task": "P14-T1-hosted-runtime-policy",
      "name": "isRealHostedRuntimeMode",
...
| `RuntimeRunnerService` artifact persistence | object store write/auth/timeout/digest failure | `Error` with object-store reason | preserve existing object-store mapping and retry/exhaustion behavior | run fails visibly or retry exhaustion is inspectable |
...
```json
{
  "exports": [
    ...
    },
    {
      ...
      "kind": "function",
      "signature": "(config: WorkerConfig, deps?: HostedWorkerAdapterFactoryDeps) => Promise<{ ok: boolean; modes: Record<string, { ok: boolean; code?: string }> }>"
    },
    {
...
- T1 is the shared contract. T2 and T3 must import hosted runtime predicates and validation helpers from `@switchyard/core`, not copy slug lists.
